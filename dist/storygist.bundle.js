...
function initHammer(selector) {
  ...
  $(selector).each(function (index, beat) {
    var hammer = new _hammerjs2.default(beat);
    hammer.on('swipe', _events2.default.swipeBeat);
    hammer.get('swipe').set({ direction: _hammerjs2.default.DIRECTION_ALL }); // enables 'Swipe Up'
  });
}
...
StoryGist.prototype.init = function () {
  ...
  _static2.default.dependencyChecker(['jQuery', 'Hammer', 'Navigo', 'SplitType', 'videojs', '$.Velocity', 'lazySizes']);
  ...
  if ($(window).width() <= self.settings.initWidth) {
    ...
    $('#gist-view-story').click(function () {
      ...
      window.removeEventListener('touchmove', _events2.default.scrollLock);
      _events2.default.viewInStory();
    });
    ...
    $pager.find('.gist-pager__btn--next').on('click', function () {
      var el = _static2.default.getCurrentBeat();
      var beatNum = _static2.default.getCurrentBeatNum();
      _events2.default.nextBeat(beatNum, el);
    });

    $pager.find('.gist-pager__btn--prev').on('click', function () {
      var el = _static2.default.getCurrentBeat();
      var beatNum = _static2.default.getCurrentBeatNum();
      _events2.default.prevBeat(beatNum, el);
    });
    ...
    $('.go-to-beginning').click(function () {
      ...
      _events2.default.goToBeginning();
    });

    $initBeat.addClass('active');
    $('.gist-beat').click(_events2.default.clickBeat.bind(self));
    ...
    window.addEventListener('scroll', function scrollListener() {
      if (window.pageYOffset >= $gistBody.offset().top) {
        ...
        window.addEventListener('touchmove', _events2.default.scrollLock);
      }
    });

    window.addEventListener('orientationchange', _events2.default.onOrientationChange);
    _events2.default.onOrientationChange();
    _events2.default.init(self);
    initHammer('.gist-beat');
    _navigation2.default.init(self);
  }
};
...
function nextBeat(beatNum, el) {
  ...
  if ($(el).hasClass('last')) {
    ...
    if (window.SplitType && $nextEl.find('.js-text-block').length && settings.animate) {
      ...
      $.Velocity(splitTextBlock.lines, _static2.default.TRANSITIONS[3], { duration: baseAnimSpeed * 0.6, stagger: baseAnimSpeed * 0.05 });
    }
    ...
    if (settings.animate) {
      ...
      if (window.SplitType && $nextEl.find('.pullquote').length) {
        ...
        $.Velocity(splitPullQuote.lines, _static2.default.getRandTransition(), { 'duration': baseAnimSpeed, 'stagger': baseAnimSpeed / 2 });
      }
    }
  ...
  _navigation2.default.navigateToBeat(+beatNum + 1, +totalBeats);
}
...
function prevBeat(beatNum, el) {
  ...
  _navigation2.default.navigateToBeat(+beatNum - 1);
}
  ...
  $.fn.storyGist = function (options) {
    return this.each(function () {
      if (undefined === $(this).data('storyGist')) {
        var plugin = new _init2.default(this, options);
        plugin.init();
